const BUILDING_TABLE_COLUMN_COUNT = 19;
  ...
  function buildPrintableBuildingTable(state, buildingName, selectedMonth) {
    ...
    return {
      ...
      <thead><tr>
        ...
        <th>Tenant</th>
        <th>Status</th>
        <th class="amount"><span class="print-header-stack"><span>Contract</span><span>amount</span></span></th>
        ...
        <th class="amount"><span class="print-header-stack"><span>Total</span><span>unpaid</span></span></th>
  ...
  async function resetCurrentBuildingMonth(state) {
    ...
    const updates = rowNodes.map((row) => {
      ...
      const prepaidLastMonthAmount = normalizeAmount(Number(tenant.prepaidNext || 0));
      ...
      return {
        ...
        paidOverride: prepaidLastMonthAmount,
        oldTenantDuePaid: 0,
        prepaidLastMonthAmount,
        prepaidAmount: 0,
        paymentMethods
      };
    }).filter(Boolean);
  ...
  function getBuildingPrepaidLastMonthAmount(state, tenant, monthKey) {
    if (!tenant || typeof getTenantMonthNumericOverride !== 'function') return 0;
  ...
  function updateBuildingFooterBankPreview(state, input) {
    ...
    const bankReceiptsTotal = previousPaidTotal
      + tenants.reduce((sum, tenant) => sum + getBuildingCurrentMonthSummaryAmount(tenant, monthKey), 0)
      + insuranceCurrentTotal
      + oldTenantDuePaidTotal;
    const prepaidTotal = tenants.reduce((sum, tenant) => sum + Number(tenant.prepaidNext || 0), 0);
    const prepaidLastMonthTotal = tenants.reduce((sum, tenant) => sum + getBuildingPrepaidLastMonthAmount(state, tenant, monthKey), 0);
    const totalCurrentMonth = bankReceiptsTotal + prepaidTotal;
    const totalInBank = normalizeAmount(totalCurrentMonth - prepaidLastMonthTotal - currentValue);
    totalsCell.textContent = formatCurrency(totalInBank);
  }
  ...
  function bindBuildingFooterInputs(state) {
    document.querySelectorAll('[data-building-footer-edit]').forEach((input) => {
      ...
      const commitFooterField = async () => {
        try {
          await saveBuildingFooterFieldFromTable(state, input);
        } catch (error) {
          console.error('Failed to save building footer value.', error);
          if (typeof showFlashMessage === 'function') {
            showFlashMessage(String(error && error.message || 'Failed to save building footer value.'));
          }
        }
      };
  ...
  async function saveBuildingFooterFieldFromTable(state, input) {
    ...
    const normalizedCurrentValue = normalizeAmount(Number(currentValue || 0));
    const normalizedInitialValue = normalizeAmount(Number(initialValue || 0));
    if (!field || !buildingName || !monthKey || input.dataset.tableFooterSaving === 'true' || normalizedCurrentValue === normalizedInitialValue) {
  ...
  function renderBuildingRow(state, tenant, selectedMonth) {
    ...
    const contractAmountFieldId = typeof buildTenantMonthFieldId === 'function'
      ? buildTenantMonthFieldId(fieldTenantId, selectedMonth, 'contract_amount')
      : '';
    const discountFieldId = typeof buildTenantMonthFieldId === 'function'
      ? buildTenantMonthFieldId(fieldTenantId, selectedMonth, 'discount')
      : '';
    ...
    const discountValue = formatBlankAmountInputValue(Number(tenant.discount || 0), allowDecimalAmounts);
    const currentMonthValue = formatBlankAmountInputValue(tenant.paidCurrent, allowDecimalAmounts);
    const contractAmountInput = `<input type="number" class="table-amount-input" step="${escapeHtml(amountInputStep)}" min="0" value="${escapeHtml(contractAmountValue)}" data-row-edit-contract="${escapeHtml(tenant.id)}" data-inline-field-id="${escapeHtml(contractAmountFieldId)}" data-initial-value="${escapeHtml(contractAmountValue)}"${isLockedBaseline ? ' readonly aria-readonly="true"' : ''}>`;
    const discountInput = `<input type="number" class="table-amount-input" step="${escapeHtml(amountInputStep)}" min="0" value="${escapeHtml(discountValue)}" data-row-edit-discount="${escapeHtml(tenant.id)}" data-inline-field-id="${escapeHtml(discountFieldId)}" data-initial-value="${escapeHtml(discountValue)}"${isLockedBaseline ? ' readonly aria-readonly="true"' : ''}>`;
    ...
    return `<tr class="${rowClasses.join(' ')}"${rowAttr}>
      ...
      <td class="center">${contractAmountInput}</td>
  ...
  function bindBuildingCurrentMonthInputs(state) {
    document.querySelectorAll('[data-row-edit-current-month], [data-row-edit-contract], [data-row-edit-discount]').forEach((input) => {
      if (input.dataset.tableAmountBound === 'true') return;
      input.dataset.tableAmountBound = 'true';
  ...
  async function saveBuildingRowFieldFromTable(state, input) {
    if (!input) return;
    const tenantId = String(
      input.getAttribute('data-row-edit-current-month')
      || input.getAttribute('data-row-edit-contract')
      || input.getAttribute('data-row-edit-discount')
      || ''
    ).trim();
    const currentValue = String(input.value || '').trim();
    const initialValue = String(input.dataset.initialValue || '').trim();
    if (!tenantId || input.dataset.tableAmountSaving === 'true' || input.readOnly || input.disabled || currentValue === initialValue) return;
    input.dataset.tableAmountSaving = 'true';
    try {
      ...
      delete input.dataset.tableAmountSaving;
    }
  }
