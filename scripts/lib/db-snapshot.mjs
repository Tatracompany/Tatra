...
export async function readDatabaseSnapshot(databaseTarget) {
  const db = await openDatabase(databaseTarget);
  try {
    ...
    const activeTenancies = await db.prepare(`
      SELECT
        t.id,
    ...
    const tenantProfiles = await db.prepare(`
      SELECT
        id,
    ...
    const tenancyHistory = await db.prepare(`
      SELECT
        t.id,
        t.profile_id AS "profileId",
        t.unit_id AS "unitId",
    ...
    const vacancyStates = await db.prepare(`
      SELECT
        unit_id AS "unitId",
    ...
    const rowOrder = await db.prepare(`
      SELECT
    ...
    const tenantMonthOverrides = await db.prepare(`
      SELECT
    ...
    const tenantMonthState = await db.prepare(`
      SELECT
        source_tenant_id AS "sourceTenantId",
        month_key AS "monthKey",
